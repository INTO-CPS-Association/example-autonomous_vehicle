...
SET FMU=%ROOTPATH%\vehicle.fmu
SET DLL=vehicle.dll
SET ZIPTOOL=C:\Program Files (x86)\20-sim 4.6\fmi-export-20sim-master\StandaloneFMU\bin\7z.exe
rem The Github hosted template includes these two tools in order to support older 20-sim versions
SET XSLTTOOL=C:\Program Files (x86)\20-sim 4.6\fmi-export-20sim-master\StandaloneFMU\bin\msxsl.exe
SET GUIDTOOL=C:\Program Files (x86)\20-sim 4.6\fmi-export-20sim-master\StandaloneFMU\bin\GenerateGuid.exe

SET PYTHON="C:\ProgramData\Controllab Products B.V\Python34\python.exe"
SET RESOURCES_SCRIPT="C:\Program Files (x86)\20-sim 4.6\fmi-export-20sim-master\StandaloneFMU\bin\include_resources.py"
...
ECHO ------------------------------------------------------------
ECHO 20-sim standalone co-simulation FMU export for 'vehicle'
ECHO ------------------------------------------------------------
...
:COMPILE_FMU
  ...
  msbuild.exe "%PROJ_DIR%\vehicle.sln" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\vehicle.sln" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:COMPILE_NO_CLEAN_FMU
  ECHO Compiling 20-sim FMU for submodel "vehicle"
  msbuild.exe "%PROJ_DIR%\vehicle.vcxproj" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\vehicle.vcxproj" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:FMU_READY
  cd ..
  ECHO Your vehicle FMU is ready and can be found in:
  ECHO   %CD%
